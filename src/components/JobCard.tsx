    ...
    <div className={`bg-white rounded-xl overflow-hidden card-hover ${
      ...
      <div className="p-6">
        <div className="flex justify-between">
          ...
          <button 
            className={`h-8 w-8 rounded-full flex items-center justify-center ${
              saved ? 'text-brand-blue bg-brand-blue/10' : 'text-gray-400 hover:text-gray-600'
            }`}
        ...
        <div className="mt-5 pt-4 border-t border-gray-100 flex justify-between items-center">
          {job.featured && (
            <span className="text-xs font-medium text-brand-teal px-2 py-1 bg-brand-teal/10 rounded-full">
              Featured
            </span>
          )}
          <div className="flex-1"></div>
          <Button size="sm" className="h-8 bg-brand-blue hover:bg-brand-darkBlue text-white">
            Apply Now
          </Button>
        </div>
      </div>
